        ...
        <div className={`space-y-6 ${isMobile ? 'pb-20' : ''}`}>
            ...
            {mode === 'gaming' && lastBooking ? (
                ...
                <div className="max-w-lg mx-auto w-full space-y-4">
                    {/* Header banner */}
                    <div className="flex items-center gap-3 bg-emerald-500/15 border border-emerald-500/40 rounded-2xl px-5 py-4">
                        <CheckCircle className="text-emerald-400 shrink-0" size={26} />
                        <div>
                            <p className="font-bold text-white text-base">Booking Confirmed!</p>
                            <p className="text-emerald-400 text-sm">{lastBooking.name}</p>
                        </div>
                    </div>

                    {/* Details table */}
                    <div className="rounded-2xl border border-slate-700 overflow-hidden bg-slate-800/60">
                        {[
                            lastBooking.cafeName ? ['🏪 Cafe', lastBooking.cafeName] : null,
                            ['📅 Date', lastBooking.date],
                            ['⏰ Time', `${lastBooking.time} (${lastBooking.duration} min)`],
                            ['🕹️ Console', lastBooking.itemsLabel],
                            ['💰 Amount', `₹${lastBooking.amount} · ${lastBooking.paymentMode}`],
                        ].filter(Boolean).map((row, i, arr) => (
                            <div key={i} className={`flex items-center justify-between px-4 py-3 ${i < arr.length - 1 ? 'border-b border-slate-700/60' : ''}`}>
                                <span className="text-slate-400 text-sm">{row![0]}</span>
                                <span className={`text-sm font-medium ${i === arr.length - 1 ? 'text-emerald-400' : 'text-white'}`}>{row![1]}</span>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {lastBooking.phone ? (
                            <a
                                ...
                                className="flex items-center justify-center gap-2 px-4 py-3 bg-[#25D366] hover:bg-[#20b558] text-white rounded-xl font-semibold text-sm transition-colors"
                            >
                            ...
                            <div className="flex items-center justify-center px-4 py-3 bg-slate-800 rounded-xl text-slate-500 text-sm">
                                No phone number
                            </div>
                        )}
                        <button
                            onClick={() => { setLastBooking(null); onSuccess?.(); }}
                            className="flex items-center justify-center gap-2 px-4 py-3 bg-slate-700 hover:bg-slate-600 text-white rounded-xl font-semibold text-sm transition-colors"
                        >
                            + New Booking
                        </button>
                    </div>
