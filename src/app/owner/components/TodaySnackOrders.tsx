    ...
    <div className="border border-[#f2f0ea]/[0.07] bg-[#f2f0ea]/[0.06]/40 overflow-hidden">
      ...
      {ordersToday.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-10 gap-2">
          <ShoppingBag size={28} className="text-[#f2f0ea]/30" />
          <p className="text-sm text-[#f2f0ea]/40">No snack orders today</p>
        </div>
      ) : (
            ...
            return (
              <div key={booking.id} className={`px-5 py-3.5 transition-colors ${booking.payment_mode === 'owner' ? 'bg-purple-500/5 hover:bg-purple-500/8' : 'hover:bg-white/[0.08]/20'}`}>
                ...
                <div className="flex flex-col gap-2 mb-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-7 h-7 rounded-full bg-white/[0.08] flex items-center justify-center shrink-0">
                      <span className="text-xs font-semibold text-[#f2f0ea]/70">
                        {customer.charAt(0).toUpperCase()}
                      </span>
                    </div>
                    <div className="min-w-0">
                      <span className="text-sm font-medium text-[#f2f0ea] truncate block">{customer}</span>
                      {phone && <span className="text-[11px] text-[#f2f0ea]/40">{phone}</span>}
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-2 sm:shrink-0 sm:ml-3">
                    {onEditSale && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-semibold text-[#f2f0ea]/50 hover:text-[#f2f0ea] hover:bg-[#f2f0ea]/[0.06] transition-colors"
                ...
                <div className="flex flex-wrap gap-1.5 pl-0 sm:pl-9">
                  ...
                  {booking.start_time && (
                    <span className="text-[11px] text-[#f2f0ea]/30 ml-1 self-center">{booking.start_time}</span>
                  )}
                </div>
              </div>
            );
